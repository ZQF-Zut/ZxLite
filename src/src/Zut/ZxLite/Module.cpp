...
{
  OpenModule::OpenModule(const ZxLite::WStrView wsModuleName)
      : m_pLDRDataEntry(OpenModule::FindLDRDataEntry(wsModuleName))
  {
  }

  OpenModule::OpenModule(const std::size_t nModuleNameHashUpperCase)
      : m_pLDRDataEntry(OpenModule::FindLDRDataEntry(nModuleNameHashUpperCase))
  {
  }
  ...
  auto OpenModule::GetProcedureBase(const std::size_t nHash) const -> void*
  {
  ...
  auto OpenModule::GetProcedure(const ZxLite::StrView msFnName) const -> void*
  {
    ...
    for (std::size_t idx = 0; idx < export_dir_ptr->NumberOfNames; idx++)
    {
      ...
      const auto compare_status = ZxLite::StrCmp(msFnName.CStr(), msFnName.Size(), func_name_cstr, func_name_len, false);
      if (compare_status != 0)
      {
        continue;
      }
