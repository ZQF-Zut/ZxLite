...
namespace ZQF::Zut::ZxLite
{
  ...
  public:
    ...
    constexpr BaseStrView(const Value_Type (&aStr)[N]) noexcept
        : m_pStr(aStr), m_nSize(N)
    {
    }
  ...
  public:
    constexpr auto CStr() const noexcept -> const Value_Type*;
  ...
  template <typename T>
  constexpr BaseStrView<T>::BaseStrView() noexcept
      : m_pStr{}, m_nSize{}
  {
  }

  template <typename T>
  constexpr BaseStrView<T>::BaseStrView(const Value_Type* pStr) noexcept
      : m_pStr(pStr)
  {
  ...
  template <typename T>
  template <typename U>
  constexpr BaseStrView<T>::BaseStrView(const Value_Type* pStr, const U nSize) noexcept
      : m_pStr(pStr), m_nSize(static_cast<Size_Type>(nSize))
  {
  }

  template <typename T>
  constexpr auto BaseStrView<T>::CStr() const noexcept -> const Value_Type*
  {
    return m_pStr;
  }

  template <typename T>
  constexpr auto BaseStrView<T>::Size() const noexcept -> Size_Type
  {
    return m_nSize;
  }

  template <typename T>
  constexpr auto BaseStrView<T>::SizeBytes() const noexcept -> Size_Type
  {
    return m_nSize * sizeof(Value_Type);
  }

  template <typename T>
  constexpr auto BaseStrView<T>::GetRtlStr() const noexcept
  {
    if constexpr (std::is_same_v<Value_Type, char>)
    {
      return ANSI_STRING{ .Length = (USHORT)this->SizeBytes(), .MaximumLength = (USHORT)(this->SizeBytes() + 1), .Buffer = (PCHAR)this->CStr() };
    }
    else if constexpr (std::is_same_v<Value_Type, wchar_t>)
    {
      return UNICODE_STRING{ .Length = (USHORT)this->SizeBytes(), .MaximumLength = (USHORT)(this->SizeBytes() + 2), .Buffer = (PWCH)this->CStr() };
    }
    else
    {
      static_assert(false, "error value type");
  ...
  template <typename T>
  template <typename U>
  constexpr auto BaseStrView<T>::FromRtlStr(const U& rfRtlStr) -> BaseStrView<Value_Type>
  {
    if constexpr (std::is_same_v<Value_Type, char>)
    {
...
namespace ZQF::Zut::ZxLite
{
  ...
  auto StrCmp(const T* pStr0, const std::size_t nStr0Len, const T* pStr1, const std::size_t nStr1Len, const bool isIgnoreCase = false) -> std::size_t
  {
    ...
    if (isIgnoreCase)
    {
      while ((ret == 0) && (bytes--)) { ret = ZxLite::CharUp(*pStr0++) - ZxLite::CharUp(*pStr1++); }
    }
    else
    {
      while ((ret == 0) && (bytes--)) { ret = *pStr0++ - *pStr1++; }
    }
  ...
  template <typename T>
  auto StrCmp(const T* pStr0, const T* pStr1, const bool isIgnoreCase = false) -> std::size_t // NOLINT
  {
