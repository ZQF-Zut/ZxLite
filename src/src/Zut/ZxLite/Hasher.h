...
namespace ZQF::Zut::ZxLite
{
  ...
  private:
    template <typename>
    struct FnV1aMagicNum
    {
    };

    template <>
    struct FnV1aMagicNum<std::uint32_t>
    {
      static constexpr std::uint32_t Prime = 16777619U;  // NOLINT
      static constexpr std::uint32_t Seed = 2166136261U; // NOLINT
    };

    template <>
    struct FnV1aMagicNum<std::uint64_t>
    {
      static constexpr std::uint64_t Prime = 1099511628211ULL;       // NOLINT
      static constexpr std::uint64_t Seed = 14695981039346656037ULL; // NOLINT
    };
  ...
  public:
    template <typename T>
    static constexpr auto HashInterger(const T tValue, const HashValueType nHash = FnV1aMagicNum<HashValueType>::Seed) -> HashValueType
    {
      HashValueType hash = nHash;
      if constexpr (std::is_same_v<T, std::uint8_t>)
      {
        hash = (tValue ^ hash) * FnV1aMagicNum<HashValueType>::Prime;
      }
    ...
    {
      HashValueType hash = FnV1aMagicNum<HashValueType>::Seed;
